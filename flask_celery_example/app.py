...
# Initialize Celery
myapp = Celery(app.name, broker=app.config['CELERY_BROKER_URL'])
myapp.conf.update(app.config)
...
@myapp.task(bind=True, ignore_result=False)
def long_task(self):
...
def taskstatus(task_id):
    ...
    app.logger.debug("task status. {0}".format(task.state))
